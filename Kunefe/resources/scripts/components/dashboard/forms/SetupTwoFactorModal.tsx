    ...
    >
      {recoveryTokens.length > 0 ?
        <>
          ...
          <pre css={tw`text-sm mt-4 rounded font-mono bg-neutral-905 p-4`}>
            {recoveryTokens.map(token => <code key={token} css={tw`block mb-1`}>{token}</code>)}
          </pre>
          <div css={tw`text-right`}>
            <Button color={'purple'} css={tw`mt-6`} onClick={dismiss}>
              Close
            </Button>
          </div>
        </>
        ...
        <Form css={tw`mb-0`}>
          ...
          <div css={tw`flex flex-wrap`}>
            <div css={tw`w-full md:flex-1`}>
              <div css={tw`w-32 h-32 md:w-64 md:h-64 bg-neutral-910 p-2 rounded mx-auto`}>
                {!token ?
                  <img
            ...
            <div css={tw`w-full mt-6 md:mt-0 md:flex-1 md:flex md:flex-col`}>
              <div css={tw`flex-1`}>
                ...
                />
                {token &&
                  <div css={tw`mt-4 pt-4 text-white`}>
                    <Label css={tw`text-xs`}>Alternatively, enter the following token into your authenticator application:</Label>
                    <CopyOnClick text={token.secret}>
                      <div css={tw`text-sm bg-neutral-910 rounded mt-2 py-2 px-4 font-mono`}>
                        <code css={tw`font-mono text-white`}>
                          {token.secret}
                        </code>
                      </div>
                    </CopyOnClick>
